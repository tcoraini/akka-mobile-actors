...
//  @Test
  def testRemotelyColocatedByClassNameActors {
    val refs = Mobile.spawn[GroupIdAnswererActor](3) at thatNode
...
//  @Test
  def testRemoteColocatedActorsNextToSingleRef {
    ...
    val controlRef = Mobile.spawn(new GroupIdAnswererActor) at thatNode
    
    while(controlRef.isLocal) { Thread.sleep(100) }

    // Refs spawned by class
    val refsC: List[MobileActorRef] = Mobile.spawn[GroupIdAnswererActor](2) nextTo controlRef
    // Refs spawned by factories
    val refsF: List[MobileActorRef] = Mobile.spawn(new GroupIdAnswererActor, new GroupIdAnswererActor) nextTo (controlRef)
...
//  @Test
  def testRemoteColocatedActorsNextToExistingGroup {
    ...
    val controlRefs = Mobile.spawn(new GroupIdAnswererActor, new GroupIdAnswererActor) at (thatNode)
    
    while(controlRefs(0).isLocal) { Thread.sleep(100) }

    // Refs spawned by class
    val refsC: List[MobileActorRef] = Mobile.spawn[GroupIdAnswererActor](2) nextTo  controlRefs(0)
    // Refs spawned by factories
    val refsF: List[MobileActorRef] = Mobile.spawn(new GroupIdAnswererActor, new GroupIdAnswererActor) nextTo (controlRefs(1))
...
//  @Test
  def testLocalColocatedActorsNextToSingleRef {
    ...
    val controlRef = Mobile.spawn(new GroupIdAnswererActor) here
...
//  @Test
  def testLocalColocatedActorsNextToExistingGroup {
    ...
    val controlRefs = Mobile.spawn(new GroupIdAnswererActor, new GroupIdAnswererActor).here
